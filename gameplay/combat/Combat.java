...
/**
 * A combat is a series of attacks. Depending on who wins "initiative", that
 * entity goes first. Adding "initiative" to "Creature" as a temporary variable
 * that shows up only during combat. There are multiple rounds which get
 * adjudicated based off of interaction from the Player(s)
 */
public class Combat {
  ...
  private boolean processAttack(Creature attacker, Creature defender) {
    ...
    if (defender instanceof Player) {
      ...
      if (attackValue <= 0) {
        attackValue = 1;
        this.console.outputMessage(
            player.getName() + "'s armor absorbed most of the damage from " + attacker.getName() + "'s attack");
      }
    }

    if (attackValue != 0) {
      this.console.outputMessage(
          attacker.getName() + " hits " + defender.getName() + " for " + attackValue + " points of damage.");
    ...
    this.console.outputMessage(defender.getName() + " has " + defender.getCurrentHitPoints() + " out of "
        + defender.getMaxHitPoints() + " hitpoints remaining.\n");
